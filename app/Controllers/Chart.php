...
class Chart extends BaseController
{
    public function index()
    {
        ...
        $dataBelanjaHarga = $data->select('MONTH(created_at) as bulan, SUM(harga) as total_jumlah_harga')
        ->where('created_at >=', $tanggalAwalTahun)
        ->where('created_at <=', $tanggalSekarang)
        ->groupBy('MONTH(created_at)')
        ->findAll();
        ...
        foreach ($dataBelanjaHarga as $belanja) {
            $bulan = (int) $belanja['bulan']; // Ambil bulan dari hasil query
            $totalJumlah = (int) $belanja['total_jumlah_harga']; // Ambil total jumlah dari hasil query
            $dataHargaPerBulan[$bulan] = $totalJumlah; // Masukkan total jumlah ke array dataPerBulan
        }
        ...
        for ($i = 1; $i <= date('n'); $i++) {
            $bulanLabelsFiltered[] = $bulanLabels[$i - 1] . ' (' . $dataPerBulan[$i] . ')';
               ...
               // Buat objek chart.js
               $JumlahChart['datahargajumlahtahun'] = [
                'type' => 'line',
                'data' => [
                    ...
                    'datasets' => [
                        [
                            ...
                            'data' => $dataHargaPerBulan, // Gunakan array data per bulan
                ...
                'datasets' => [
                    ...
                    'options' => [
                        'scales' => [
                            ...
                            $dataHargaBelanjaBulan = $data->select('DATE(created_at) as tanggal, SUM(harga) as total_jumlah_harga')
                            ->where('created_at >=', $tanggalSatuBulanLalu)
                            ->where('created_at <=', $tanggalSekarang)
                            ->groupBy('DATE(created_at)')
                            ->findAll();
                            ...
                            foreach ($dataHargaBelanjaBulan as $belanja) {
                                $tanggal = date('Y-m-d', strtotime($belanja['tanggal'])); // Konversi tanggal ke format 'Y-m-d'
                                $bulanLabels[] = $tanggal; // Tambahkan tanggal ke array label
                                $dataHargaPerTanggal[] = (int) $belanja['total_jumlah_harga']; // Tambahkan total jumlah belanja ke array data
                            }

                            $JumlahChart['datahargajumlahbulan'] = [
                                ...
                                'data' => [
                                    ...
                                    'datasets' => [
                                        [
                                            ...
                                            'data' => $dataHargaPerTanggal, // Gunakan array data per tanggal
                                            'backgroundColor' => 'rgba(75, 192, 192, 0.2)',
                                            'borderColor' => 'rgba(75, 192, 192, 1)',
                                            'borderWidth' => 1,
                                            ]
                                    ...
                                    'datasets' => [
                                        ...
                                        'options' => [
                                            'scales' => [
                                                ...
                                                $dataHargaBelanja = $data->select('DATE(created_at) as tanggal, SUM(jumlah) as total_jumlah_harga')
                                                ->where('created_at >=', $tanggalSatuMingguLalu)
                                                ->groupBy('DATE(created_at)')
                                                ->findAll();
                                                ...
                                                foreach ($dataHargaBelanja as $data) {
                                                    $labels[] = $data['tanggal']; // Memasukkan tanggal ke array label-label
                                                    $dataHargaPerTanggal[] = $data['total_jumlah_harga']; // Memasukkan data jumlah belanja ke array data per tanggal
                                                }

                                                $JumlahChart['datahargajumlahminggu'] = [
                                                    'type' => 'line',
                                                    'data' => [
                                                        ...
                                                        'datasets' => [
                                                            [
                                                                'label' => 'Total Belanja per Tanggal',
                                                                'data' => $dataHargaPerTanggal, // Menggunakan array data jumlah belanja per tanggal
                                                                'backgroundColor' => 'rgba(75, 192, 192, 0.2)',
                                                                'borderColor' => 'rgba(75, 192, 192, 1)',
                                                                'borderWidth' => 1,
                                                                ]
